...
Breakwater and Flowsafe require Node 22.13.0 or later because their required `@mastra/core` `1.67.0` peer has that runtime floor. Fleet Control requires Node 22.22.0 or later. All three packages are ESM-only. React 18 or 19 is needed only for the optional Flowsafe approval UI. Hosts that run `flowsafe-provision` or the Fleet Control Wrangler backend must provide Wrangler `>=4.118 <5`; Wrangler is not a Flowsafe peer dependency.

## Start with breakwater

```bash
npm install @mastra/core@1.67.0 @proofoftech/breakwater
```
...
```bash
npm install @mastra/core@1.67.0 @proofoftech/breakwater @proofoftech/flowsafe
```
