...
| `wrangler.jsonc` | Deploy config — `RUNNER` DO + `DB` D1 bindings, TWO cron triggers (keep byte-equal to worker.ts's `SWEEP_CRON`/`PURGE_CRON`), SLA/retention vars, commented `queues` block for audit export, secret docs | Changing bindings, cron cadence, or defaults |
