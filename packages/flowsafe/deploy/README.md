...
- `purgeExpiredWorkflowRuns()` scans terminal snapshots and spent start keys in bounded pages. The maintenance Durable Object persists its scan cursor across alarms and restarts.
...
Custom purge callers must provide transactional `database.batch()`, persist `advanceCursor` updates, and pass the saved cursor into subsequent calls. Cursor persistence failure stops the run-retention phase; sibling purge duties keep their independent failure handling. See [Retention](../../../docs/do-runner-design.md#retention) for the direct-call contract and generation/namespace limits.

If storage uses `tablePrefix`, configure the identical value as `storageTablePrefix` on `createFlowsafeWorker()`. Use the prefix contract documented by `createD1Storage()`. A valid retention cursor from a different namespace configuration starts a fresh managed scan; a malformed cursor records a retention failure.
