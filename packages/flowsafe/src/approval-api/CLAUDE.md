...
| `d1-store.ts` | `D1ApprovalStore`: schema DDL (partial unique open-step index, `suspended_at` column with a defensive ALTER backfill for pre-1.0 spike DBs), `UPDATE ... RETURNING` CAS, lazy memoized schema creation (`#ready`/`#createSchema`); `ApprovalDatabase` structural D1 subset | Changing the schema or SQL |
