...
| `host.ts` | `BackgroundTaskHost`. Persistence-only mode retains the existing recovery behavior. Execution mode validates the serialized and tenant-scoped domains, registers executors, initializes the manager, and starts public Mastra workers on the same pubsub | Hosting the manager or changing its boot and shutdown lifecycle |
