...
| `host-kit/` | Host-agnostic glue every host shares. `./host-kit` (barrel, deliberately **breakwater-free**): the identity seam (`TokenVerifier` — `staticTokenVerifier` + HS256 `hmacVerifier`/`mintHmacToken` in `verifier.ts`; `parseActorTokens` requires an INV-3 `tenantId` per entry; `bearerActorAuthenticator(verifier)`), `createRunRouter({ resolve })` (401 → INV-3 → coarse `RUN_START_ROLES` → per-workflow `allowedRoles`; rejects client runIds and mints `${tenantId}_${uuid}`; 404s other tenants' runs on status/resume — no existence oracle), the tenants allocation registry (`tenant-registry.ts`: `provisionTenant` insert-or-fail + `RESERVED_TENANT_SLUGS`), the subdomain↔tenant cross-check (`subdomain-check.ts`), `RunRouteError` + `doSummary`, `assertWorkflowsRegistered`, the approval bridge, and `WorkflowMeta`. `./host-kit/module` (separate subpath): the `WorkflowModule` authoring contract | Changing the shared run routes, identity seam, tenant registry, or the suspension→approval bridge |
