...
| `runtime.ts` | `RunnerRuntime` — register/start/resume/status/workflowIds, per-run FIFO lock, `PATH_SAFE_ID_PATTERN` validation (runId at `start()`, workflowId at `register()`), error taxonomy, the `requestContextForRun` provider seam (`RunLeg` with resumed step + `suspendedAt` + `resumeCount`, the runtime-owned monotonic resume ordinal that is the grant-binding tie-breaker; every leg mints the breakwater workflow-scope key, provider values merge over it), the in-memory per-run resume ledger (`#resumeCounts`) feeding `RunSummary.resumeCount`, widened `status()` projection incl. `RunSummary.suspendedAt`/`resumeCount` (per-step); rationale lives in the module's doc comments | Changing run lifecycle, id validation, locking, error mapping, or the requestContext seam |
