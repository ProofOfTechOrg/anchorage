...
| `src/token-gate.tsx` | `TokenGate` (signed-out landing: tagline, sandbox promise, OAuth demo entry + operator token paste — inline when it is the only entry, behind an "operator API token" modal when the OAuth demo leads; the entry section HOLDS while the tri-state /auth/config probe is in flight so the OAuth landing never paints the paste layout and swaps it) + `OperatorIdentityChip` | Changing sign-in UI |
