...
| `src/showcase-panels.tsx` | `TokenGate` (operator sign-in + optional OAuth demo entry — href/label derive from the server-echoed provider name via `demoSignInProvider`), `LauncherPanel` (start any of the 5; role gates render from the SERVER'S catalog actor echo — `actorForToken`/`DEFAULT_ACTOR` are gone, an unknown token renders as nothing, never admin), `RunStatusPanel` (self-scheduling poll: surfaces a failed status READ as `unavailable`, stops on a hard `RunApiError` or after `MAX_TRANSIENT_FAILURES`). Holds NO token table and must not import demo-actors. All render through `useApprovalUIComponents()` slots | Changing the launcher/status/sign-in UI |
