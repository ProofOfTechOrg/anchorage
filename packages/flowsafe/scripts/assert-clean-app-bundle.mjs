...
// The tokens are public dev credentials (showcase/demo-actors.ts); shipping
...
const distDir = join(dirname(fileURLToPath(import.meta.url)), '../app/dist');
...
if (checked === 0) {
  console.error('assert-clean-app-bundle: no bundle files found in app/dist');
  process.exit(1);
}
if (hits.length > 0) {
  console.error(
