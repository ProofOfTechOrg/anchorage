...
| `CHANGELOG.md` | Release notes (0.2.0 released 2026-07-11; breakwater is an optional peer consumed only by `./host-kit/module` — peer range `>=0.2.0 <1.0.0` so breakwater 0.x minors never trip changesets' major escalation, see root CLAUDE.md "Branching and releases") | Cutting a release |
...
| `package.json` | Manifest, subpath exports (`./agent-runner`, `./approval-api`, `./approval-ui`, `./artifacts`, `./audit-export`, `./do-runner`, `./goals`, `./host-kit`, `./schedules`, `./signal-providers`, `./signals`), scripts (`spike`/`spike:verify`, `deploy:cf`/`deploy:dev`; build/typecheck run the extra approval-ui passes incl. the UI TEST pass `src/approval-ui/tsconfig.test.json`, plus `spike/` + `deploy/` tsc passes; `typecheck:react18` compiles the BUILT approval-ui dist against the aliased `types-react-18` devDeps with `skipLibCheck: false` — the react-18 peer-floor probe, needs a prior build, also a CI step). Sole runtime dep is `@mastra/cloudflare-d1`. Optional react peers (React 18+); breakwater devDep (tests only); happy-dom devDep (the one renderer-backed hook regression test only). The Astryx-styled demo app lives in the separate `showcase` package — published consumers pull zero Astryx | Adding a subpath export, changing scripts, bumping deps |
| `typedoc.json` | Package typedoc entries (root barrel + host-kit + host-kit/module over the build tsconfig) — consumed by the root `pnpm docs:api`. approval-ui is deliberately excluded (its DOM/JSX program conflicts with the workers-typed one; the approval-ui README documents that surface) | Changing what the API reference covers |
| `scripts/react18-probe/` | `consumer.tsx` + tsconfig for `typecheck:react18` — compile-only consumer of `dist/approval-ui` under `@types/react@18` (the peer floor), `skipLibCheck` OFF so d.ts drift fails loudly | Changing the react-18 compatibility guarantee |
