...
`wrangler.jsonc` has an `assets` block whose client directory is generated by the Vite build (`not_found_handling: single-page-application`), with `run_worker_first` keeping the API routes (`/api/*`, `/runs`, `/runs/*`, `/healthz`, `/workflows`, and `/auth/*`) on the Worker. Two cron expressions are declared and dispatched on `controller.cron`, so the SLA sweep and the purge never share an invocation (a CPU-limit termination kills the isolate and cannot be caught, so a slow sweep would starve the purge forever).
