...
The `.dev.vars` copy is not optional. It supplies the local-only internal Durable Object credential and `APPROVAL_ACTOR_TOKENS`; without them requests fail closed. The provisioning command seeds the same local state directory that `dev:worker` opens. (`pnpm dev` needs neither step because the in-process host reads its identities from `worker/demo-actors.ts` and does not use workerd bindings.)
...
the API routes (`/api/*`, `/runs`, `/runs/*`, `/healthz`, `/workflows`, and
`/auth/*`) on the Worker. Two cron
