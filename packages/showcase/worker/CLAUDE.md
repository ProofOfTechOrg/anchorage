...
| `worker.ts` | Host only, now a THIN SHELL over host-kit's `createFlowsafeWorker` — this file supplies `ShowcaseRunner` (grant store bound to the DO's OWN tenant via `this.tenantId`), `buildVerifier` (static map + demo JWTs; memoized per isolate on its env inputs; kill-switch aware — `DEMO_DISABLED` parses fail-CLOSED via `boolVar`), the two cron literals, and the composer hooks: `preRoutes` (the `/auth/*` mount with env-selected provider — Google when its FULL id+secret pair is set, else GitHub's; a half-set pair is a config-error and stays unmounted — then the `/demo/reset` mount over the SAME `kit.resolve`; run budget deliberately NOT refilled), `wrapStart`/`wrapResume` (the demo-budget charge on run starts AND raw resumes; cap vars parse with `allowZero` so `=0` is a real freeze), and `extraPurgeDuties` (the demo-tenant reaper, own try/catch so its error surface stays `demo-tenant-purge`). Also the D1-backed connector rate-limit store (`D1RateLimitStore` per isolate — an in-memory window under DO-per-run routing is a per-RUN budget). The fetch pipeline, resolver assembly, two-cron dispatch, and `queue()` handler live in the composer | Changing the DO topology, maintenance, auth composition, or bridge wiring |
