  ...
  return {
    ...
    async fetch(input, init) {
      ...
      const aborted = new Promise((_, reject) => {
        abort = () => reject(new DirectBootstrapError('provider-unavailable'));
...
export async function bootstrapDirectConformance(input) {
  ...
  const transport = providerTransport(
    fetchRequest,
    prepared.config.referenceWorker.requestTimeoutMs,
  );
  try {
    ...
    const sdk = new Cloudflare({
      ...
      userServiceKey: null,
      logLevel: 'off',
      timeout: prepared.config.referenceWorker.requestTimeoutMs,
      maxRetries: 0,
