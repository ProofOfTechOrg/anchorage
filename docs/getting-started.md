...
The structural D1 types accept a Cloudflare `D1Database`; type the example's `db` accordingly in your Worker. The migration acknowledgement is valid for this new empty deployment only. Before setting it on an upgraded shared store, stop and drain every legacy writer and inventory the legacy keys as described in [Connector interface](connector-interface.md#idempotency). Use shared D1 stores when a rate limit or idempotency key must hold across isolates. In-memory stores cover only one isolate. Flowsafe connector budgets and idempotency are deployment-wide. A different generic host can use Breakwater's opaque isolation scope when it owns another trusted logical partition.
