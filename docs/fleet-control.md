...
Remove them with the [purge script](../packages/fleet-control/scripts/direct-credentialed-purge.mjs): with the run's `FLEET_DIRECT_CONFORMANCE_CONFIG`, `CLOUDFLARE_ACCOUNT_ID`, and `CLOUDFLARE_API_TOKEN` set, `node packages/fleet-control/scripts/direct-credentialed-purge.mjs` lists every custom domain, route, Worker script, Durable Object namespace, D1 database, and R2 bucket whose name or owning script begins with the configuration's `resourcePrefix`. `--delete <resourcePrefix>`, with the prefix repeated exactly, removes custom domains and routes, deletes the Worker scripts and their Durable Object namespaces, deletes the D1 databases, empties and deletes each bucket, then performs a complete rescan. It confirms the account through the provider before its first deletion and deletes nothing the residual scan would not count; a prefixed queue or dispatch namespace is reported and left, since the lane creates neither. The token needs no permission beyond the run's own. Delete `.direct-conformance/<resourcePrefix>/` afterwards; a resume after a purge refuses during pending-invocation reconciliation or bootstrap revalidation.
