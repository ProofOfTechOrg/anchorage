...
At commit `308bd39`, dated 2026-09-09, the unminified namespace-import fixture measured 3,292,290 raw bytes and 448,465 gzip bytes in the supported and historical configurations, with zero size delta. Its repository regression budgets are 4,128,768 raw bytes and 589,824 gzip bytes: 25% headroom rounded upward to 64 KiB. Those byte figures are that dated capture rather than a standing property of the package: `pnpm test:packed-fleet-control` re-measures the fixture and prints the raw and gzip bytes it read beside the budgets it enforces.
...
Active-route attestation starts from provider routing state and refuses an operation that cannot confirm the expected artifact. See [active-route attestation](security-threat-model.md#active-route-attestation) for the threat premise and promotion window.

Initial provisioning, the migration plan's promote-and-settle steps — reached by `migrateFleet()` and by `advanceFleetMigration()` — and `rollbackExternalRelease()` attest after promotion, reaching `attestConvergedActiveRoute()` through `settlePromotedRoute` on the migration and rollback paths, and fail closed when the route is absent, ambiguous, or mismatched.
...
Two optional call-local options ride on every call: `signal` cancels cooperatively, and `onComplete` is delivered at least once for a finalized operation, so the host deduplicates on `operationId`. `AdvanceFleetMigrationOptions` declares both contracts in full.
...
- Item page limits from 1 through 1,000; a larger `limit` is served at the ceiling — see [Audit an account under a request budget](#audit-an-account-under-a-request-budget). Explicit prune limits from 1 through 1,000, refused outside that range
...
Two limitations are deliberate. First, a generation is a point-in-time-per-stage snapshot, not a globally consistent one: a resource that changes between stages — a script deleted after the script listing but before its detail read — is recorded exactly as the single-call drain surfaces it, through the same `incomplete-deployment` finding. Second, durable findings never echo transient provider text. The two sites that previously interpolated a provider error store the fixed details `registered script '<name>' could not be inspected` and `plain Worker '<name>' could not be inventoried`; the transient text stays call-local, which is why `collectFleetInventory()` can still compose today's exact bytes while the durable row cannot.
...
No-export deletion requires durable proof that candidate code never gained execution authority. See [bounded no-export cleanup](security-threat-model.md#bounded-no-export-cleanup) for admissibility and refusal rules.

Trusted platform-authored deployments therefore keep no-export cleanup through `worker-deployed`.
...
Use the repository's direct-API lane to verify ordinary Worker provisioning, migration interruption and resumption, execution-fence proofs, and teardown. The offline acceptance runs the real runtime through a fixture provider and local workerd, starting from confirmed bootstrap receipts. It exercises bootstrap revalidation and teardown; resource creation and live provider behavior require the credentialed lane. No live acceptance is established until you supply credentials and run it against an account: offline results establish no live resource creation, no live account cleanup, and no recovery authorization.

The scenario force-decommissions role `a`'s terminal row and records that the call issues no provider request. The inventory proof compares the custom-domain hostnames routed to each tenant script before and after migration.

The direct scenario then provisions role `a` again under an explicit `reprovision` cycle. Its tenant Worker starts a server-assigned FlowSafe workflow run under provider version A, suspends at the declared `hold` step, advances the execution fence to `migration-locked`, and migrates that same database, script, route, and Durable Object namespace tuple to provider version B. A locked resume must return `503` with `EXECUTION_FENCED` and `migration-locked`; reopening permits the same run id to finish with its original challenge and `APPLICATION_RELEASE=2`. The resume mutation decides the approval filed by the suspension before it proves the work inventory empty. This disposable conformance tenant has one static-token actor, so it sets `APPROVAL_ALLOW_SELF_DECISION=true` and records that separation of duties is disabled for this proof. A and B are distinct provider versions at 100 percent and carry their expected deployment-spec digests, but use identical tenant module bytes. The scenario decommissions the replacement through its cycle-selected operation and verifies a third export receipt before reference teardown. Platform run tokens and gateway identity are not Anchorage-owned proof objects under the cp30 R1 boundary and are recorded as not applicable rather than replaced with configuration or reference identifiers.

If a crash leaves a prepared continuation start, the journal records terminal `proof-unavailable` with `lost-run-id-abandoned`. Any other prepared scenario call records `prepared-invocation-abandoned`. A resume revalidates bootstrap, then runs a lifecycle sweep before teardown can delete the reference Worker or Fleet database. The sweep removes recovery first through its force path when force evidence exists, then cleans up or decommissions roles `a` and `b` through their frozen lifecycle slots. A complete sweep lets reference teardown proceed. Successful cleanup still exits failed because the scenario proof is incomplete, and acceptance's three-receipt comparison fails. A remaining prefixed provider surface produces `residual-present` and exit `4`.

The sweep refuses before reference teardown deletes anything when lifecycle state or force evidence is unsupported or inconsistent, a cleanup admission rejects the record, a frozen operation or prerequisite does not match, application R2 cannot be proven empty, lifecycle reconciliation cannot attest identity, an operation blocks, the invocation budget is exhausted, or an answer is not recognized. Use the retained reference control plane to inspect the journal and force-decommission those deployments.

The sweep does not recover every interruption. A resume refuses `outcome-unknown` when a process loss fell between the sweep's prepared call and the journal's reservation, `prerequisite-unavailable` when the sweep was interrupted between an export object deletion and the end of that role's decommission, and a loss inside a bootstrap provider mutation is not re-probed; a refusal raised before any role is classified is filed against `recovery`. In each case the journal stays inspection-only and the run's resources stay in the account.

Remove them with the [purge script](../packages/fleet-control/scripts/direct-credentialed-purge.mjs): with the run's `FLEET_DIRECT_CONFORMANCE_CONFIG`, `CLOUDFLARE_ACCOUNT_ID`, and `CLOUDFLARE_API_TOKEN` set, `node packages/fleet-control/scripts/direct-credentialed-purge.mjs` lists the custom domains, routes, Worker scripts, Durable Object namespaces, D1 databases, and R2 buckets returned by the provider's listings whose name or owning script begins with the configuration's `resourcePrefix`. Its summary reports which surfaces the provider corroborated as complete: each surface in `before` and `after` carries `exhaustive`. Every summary that carries `before` carries `uncorroborated`: it names the surfaces whose flag is `false` in `after`, or in `before` when `after` is `null`, and it is `null` in a provider-failure summary whose `before` is `null`. `--delete <resourcePrefix>`, with the prefix repeated exactly, removes custom domains and routes, deletes the Worker scripts and their Durable Object namespaces, deletes the D1 databases, empties and deletes each bucket, then rescans the same surfaces. It confirms the account through the provider before its first deletion and deletes nothing the residual scan would not count; a prefixed queue or dispatch namespace is reported and left, since the lane creates neither. The token needs no permission beyond the run's own. Delete `.direct-conformance/<resourcePrefix>/` afterwards; a resume after a purge refuses during pending-invocation reconciliation or bootstrap revalidation.
...
`referenceWorker.maxProviderRequests` counts provider, maintenance, and application HTTP attempts within one invocation and may not exceed `referenceWorker.subrequestLimit`, the platform subrequest limit.

The sweep does not raise the scenario invocation floor. Its normal tenant work fits within the removal-phase reserves that the abandoned run did not spend, recovery uses the existing force, observation, and residual-recovery reserves, and bootstrap adds one control read. If the remaining run budget cannot fund those calls, the sweep records `invocation-budget-exhausted` and leaves the control plane intact.
...
Validate the local configuration and artifacts before supplying credentials. The default mode is `--preflight`; it constructs no SDK client, takes no run lock, writes no evidence, and needs no built package output:

```bash
node packages/fleet-control/scripts/direct-credentialed-conformance.mjs --preflight
```

The workspace script builds the package first, which is what the live modes below need:
