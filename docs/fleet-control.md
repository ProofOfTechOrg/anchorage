...
Remove them with the [purge script](../packages/fleet-control/scripts/direct-credentialed-purge.mjs): with the run's `FLEET_DIRECT_CONFORMANCE_CONFIG`, `CLOUDFLARE_ACCOUNT_ID`, and `CLOUDFLARE_API_TOKEN` set, `node packages/fleet-control/scripts/direct-credentialed-purge.mjs` lists the custom domains, routes, Worker scripts, Durable Object namespaces, D1 databases, and R2 buckets returned by the provider's listings whose name or owning script begins with the configuration's `resourcePrefix`. Its summary reports which surfaces the provider corroborated as complete. `--delete <resourcePrefix>`, with the prefix repeated exactly, removes custom domains and routes, deletes the Worker scripts and their Durable Object namespaces, deletes the D1 databases, empties and deletes each bucket, then rescans the same surfaces. It confirms the account through the provider before its first deletion and deletes nothing the residual scan would not count; a prefixed queue or dispatch namespace is reported and left, since the lane creates neither. The token needs no permission beyond the run's own. Delete `.direct-conformance/<resourcePrefix>/` afterwards; a resume after a purge refuses during pending-invocation reconciliation or bootstrap revalidation.
