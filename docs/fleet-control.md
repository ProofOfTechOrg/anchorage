...
- Give every production `CloudflareProvisioningClient` a `D1CloudflareApiRateCoordinator` backed by one shared direct Workers D1 binding. Use the same explicit nonsecret `quotaScope` for every replica and account sharing a Cloudflare user/account-token quota; never substitute a REST-backed D1 adapter
