...
- **Fleet API budget.** The global Cloudflare API limit is 1,200 requests per 5 minutes per account token ([Workers for Platforms limits](https://developers.cloudflare.com/cloudflare-for-platforms/workers-for-platforms/platform/limits/)); `CloudflareProvisioningClient` defaults to a 1,100-request rolling five-minute cap.
